...
import asyncio
import danmaku
from huyagift import Dbget


async def printer(q,dbget):
    while True:
        m = await q.get()
        dbget.insertInto(m["type"],m["name"])
        #print(m)
...
async def main(url):
    ...
    asyncio.create_task(printer(q,dbget))
    await dmc.start()


#a = input('请输入直播间地址：\n')
asyncio.run(main('https://www.huya.com/518512'))
